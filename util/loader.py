...
def readConfig(config_file_name="/media/sqpang/新加卷/rada_target_detection/code/RADDet_Pytorch-main(1)/RADDet_Pytorch-main/config.json"):
...
anchors_path = os.path.join(dir_path, "anchors.json")
...
def getSequenceNumbers(radar_dir, data_format):
    ...
    for this_file in glob(os.path.join(radar_dir, "*" + data_format[-1])):
        digits = int(
            "".join((s for s in this_file.replace(radar_dir, "") if s.isdigit()))
        )
...
def gtfileFromRADfile(RAD_file, prefix, input_type):
    """Transfer RAD filename to gt filename"""
    RAD_file_spec = RAD_file.split(input_type)[-1]
    gt_file = os.path.join(prefix, "gt") + RAD_file_spec.replace("npy", "pickle")
    return gt_file , RAD_file_spec


def imgfileFromRADfile(RAD_file, prefix, input_type):
    """Transfer RAD filename to gt filename"""
    RAD_file_spec = RAD_file.split(input_type)[-1]
...
def imgfileFromRADfile_new(RAD_file, prefix, input_type):
    """Transfer RAD filename to gt filename"""
    RAD_file_spec = RAD_file.split(input_type)[-1]
    gt_file = os.path.join(prefix, "stereo_image") + RAD_file_spec.replace("npy", "jpg")
    return gt_file
...
def gtfileFromRADfile_new(RAD_file, prefix, input_type):
    """Transfer RAD filename to gt filename"""
    RAD_file_spec = RAD_file.split(input_type)[-1]
    gt_file = os.path.join(prefix, "gt") + RAD_file_spec.replace("npy", "pickle")
    return gt_file
...
def getRADFromRD(RD_file, prefix, input_type):
    """Transfer RAD filename to gt filename"""
    RAD_file_spec = RD_file.split(input_type)[-1]
    RAD_file = os.path.join(prefix, "RAD") + RAD_file_spec
    return RAD_file