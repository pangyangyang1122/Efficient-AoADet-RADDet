...
def drawCartPredWithGt( RA_cart_img, gt_instances, all_classes, gt_colors, pred_colors, axes, radar_cart_nms=None):
    """ draw predict and gt boxes on the RA cartsian images """

    assert len (gt_instances["boxes"]) == len (gt_instances["classes"])
    for i in range(len (gt_instances["classes"])):
        bbox3d = gt_instances["boxes"][i]
        cls = gt_instances["classes"][i]
        cart_box = gt_instances["cart_boxes"][i]
        gt_color = gt_colors[all_classes.index(cls)]
        ### draw box
        mode = "box" # either "box" or "ellipse"
        ### boxes information added in the ground truth dictionary
        cart_box = np.array([cart_box])
        ### draw boxes
        drawBoxOrEllipse(cart_box, cls, axes[0], gt_color, \
                        x_shape=RA_cart_img.shape[1], mode=mode, if_facecolor=True)
    ...
    for i in range(len(radar_cart_nms)):
        ...
        cls = int(radar_cart_nms[i, 5])
        class_name = all_classes[cls]
        pred_color = pred_colors[int(cls)]
        mode = "box" # either "box" or "ellipse"
        drawBoxOrEllipse(cart_box, class_name, axes[0], pred_color, \
                            x_shape=RA_cart_img.shape[1], mode=mode)
    
    ####画出图像####
    imgPlot(RA_cart_img, axes[0], None, 1, "Cartesian")
def drawCartPredWithGt( RA_cart_img, gt_instances, all_classes, gt_colors, pred_colors, axes, radar_cart_nms=None):
    """ draw predict and gt boxes on the RA cartsian images """
