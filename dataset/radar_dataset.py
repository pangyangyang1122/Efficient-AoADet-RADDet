...
class RararDataset(Dataset):
    def __init__(
        ...
        self.RAD_sequences_train, self.RAD_sequences_validate = self.splitTrain(
            self.RAD_sequences_train
        )
        self.batch_size = config_train["batch_size"]
        self.total_train_batches = (
            self.config_train["epochs"] * len(self.RAD_sequences_train)
        ) // self.batch_size
        self.total_test_batches = len(self.RAD_sequences_test) // self.batch_size
        self.total_validate_batches = (
            len(self.RAD_sequences_validate) // self.batch_size
        )
        self.dtype = dType
    ...
    def trainData(self, index):
        ...
        while not has_label:
            ...
            RAD_data = (
                RAD_data - self.config_data["global_mean_log"]
            ) / self.config_data["global_variance_log"]
            ### load ground truth instances ###
            gt_filename = loader.gtfileFromRADfile(
                RAD_filename, self.config_data["train_set_dir"]
            )
    ...
    def testData(self, index):
        ...
        while not has_label:
            ...
            ### load ground truth instances ###
            gt_filename = loader.gtfileFromRADfile(
                RAD_filename, self.config_data["test_set_dir"]
            )
            gt_instances = loader.readRadarInstances(gt_filename)
            if gt_instances is None:
                raise ValueError("gt file not found, please double check the path")
    ...
    def readSequences(self, mode):
        ...
        if mode == "train":
            sequences = glob.glob(
                os.path.join(
                    self.config_data["train_set_dir"], f"{self.RADDir}/*/*.npy"
                )
            )
        else:
            sequences = glob.glob(
                os.path.join(self.config_data["test_set_dir"], f"{self.RADDir}/*/*.npy")
            )
        if len(sequences) == 0:
            raise ValueError(
    ...
    def encodeToLabels(self, gt_instances):
        ...
        for i in range(len(gt_instances["classes"])):
            ...
            iou_scaled = helper.iou3d(
                box_xyzwhd_scaled, anchorstage_xyzwhd, self.input_size
            )
            ...
            if not exist_positive:
                ...
                gt_labels[xind, yind, zind, anchor_ind, 6:7] = 1.0
