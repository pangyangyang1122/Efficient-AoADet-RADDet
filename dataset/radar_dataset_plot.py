...
class RararDatasetEvaluate(Dataset):
    def __init__(self, config_data, config_train, config_model, config_radar, headoutput_shape,
                 anchors, transformer=ToTensor(), anchors_cart=None, cart_shape=None, dType="train", RDDir="RD", RADDir="RAD"):
        ...
        self.RAD_sequences_test = self.readRADSequences(mode="test")
        self.RD_sequences_test = self.readRDSequences(mode="test")
    ...
    def __getitem__(self, index):
        if self.dtype == "test":
            return self.testData_cart(index)
        else:
            raise ValueError("This type of dataset does not exist.")
    ...
    def testData_cart(self, index):
        ...
        while not has_label:
            ...
            gt_file = loader.gtfileFromRADfile_new(RD_filename, self.config_data["test_set_dir"],
                                                   self.config_data["input_type"])
            gt_instances = loader.readRadarInstances(gt_file)

            gt_labels_cart, has_label_cart, raw_boxes_cart = self.encodeToCartBoxesLabels(gt_instances)

            raw_boxes_cart = torch.unsqueeze(torch.tensor(raw_boxes_cart, dtype=torch.float32), dim=0)
            return  normal_RD, RA_cart_img, raw_boxes_cart, RAD_filename, gt_instances
    ...
    def readRADSequences(self, mode):
    ...
    def readRDSequences(self, mode):
        """ Read sequences from train/test directories. """
        assert mode in ["train", "test"]
        if mode == "train":
            sequences = glob.glob(os.path.join(self.config_data["train_set_dir"], f"{self.RDDir}/*/*.npy"))
        else:
            sequences = glob.glob(os.path.join(self.config_data["test_set_dir"], f"{self.RDDir}/*/*.npy"))
        if len(sequences) == 0:
            raise ValueError("Cannot read data from either train or test directory, "
                             "Please double-check the data path or the data format.")
        return sequences
