...
import sys
sys.path.append('/media/sqpang/新加卷/rada_target_detection/code/Finalmodel_LinearFFT+Detection/')
...
def plot_pred_RAD(pred_real_DRA,config_data,RAD_file_spec):
    save_dir = "/media/sqpang/新加卷/rada_target_detection/Data/RADDet_reprocessed/DEbug/test/RAD_predict_FFTParams_initialization_lr=0.001"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    file_name = os.path.join(save_dir, RAD_file_spec[0].split("/")[-1].replace("npy", "png"))
    
    RAD_pred = pred_real_DRA.permute(0,2,3,1)
    ...
    RAD_mag = RAD_pred * (RAD_mag_max - RAD_mag_min) + RAD_mag_min

    RA = helper.getLog(helper.getSumDim(pow(RAD_mag,2), target_axis=-1), scalar=10, log_10=True)
    RD = helper.getLog(helper.getSumDim(pow(RAD_mag,2), target_axis=-2), scalar=10, log_10=True)
    ...
    gs = gridspec.GridSpec(1, 2, width_ratios=[4,1], wspace=0.1)  # width_ratios=[4, 1] 指定第二个子图的宽度为第一个的1/4，wspace=0.1 减少子图之间的空白
...
def draw_predAndgtOn_cart(config_data, model_cart, radar_dataset,
                    config_evaluate, config_model, input_size, anchor_boxes):
    """ Plot the predictions and gt on RA Cartesian map in dataset """
    
    fig, axes = drawer.prepareFigure(1, figsize=(10, 5))
    ...
    save_dir = os.path.join(config_data["test_set_dir"],"Efficient_AoADet_w/o_atten_pred_gt_0.7")
    ...
    for data, RA_cart_img, raw_boxes_cart, RAD_filename, gt_instances in radar_dataset:
        name = RAD_filename.split("/")[-1].replace("npy", "png")
        
        data = transformer(data).unsqueeze(0).to(device)
        
        pred_raw_cart, pred_DRA = model_cart(data)
        pred_cart = model_cart.decodeYolo(pred_raw_cart)
        pred_cart = pred_cart.cpu().detach().numpy()
        pred_frame_cart = pred_cart[0]
        predicitons_cart = helper.yoloheadToPredictions2D(pred_frame_cart,      #将类别解码为类别编号，并滤出置信度小于与值的框
                                                        conf_threshold=0.7)
        ...
        drawer.drawCartPredWithGt(RA_cart_img, gt_instances, config_data["all_classes"], gt_colors, pred_colors, axes, radar_cart_nms=nms_pred_cart)
        # dataID = os.path.splitext(os.path.basename(RAD_file))[0]
        drawer.saveFigure(save_dir, name)
...
def test_step_cart(config_model, config_data, config_evaluate, model_cart, test_dataloader, num_classes, map_iou_threshold_list):
    ...
    for data, label, raw_boxes, RAD_file_spec in test_dataloader:
        data = data.to(device)
        label = label.to(device)
        raw_boxes = raw_boxes.to(device)
        pred_raw, pred_real_DRA = model_cart(data)
        #####################画出预测的RA，RD图以及标签图###########################
        ###BS要设为1
        # plot_pred_RAD(pred_real_DRA,config_data, RAD_file_spec)
        ########################################################################
...
def main(args):
    ...
    # model_RADec = copy.deepcopy(model.RA_decoder) #没有用FPN ，没有增加预测数量
    # model.to(device)

    model_cart = RADDetCart(config_model, config_data, config_train, anchor_boxes_cart, device, config_model["input_shape"], config_model["bk_output_size"],
                            pos_encode=config_model["position_encode"], backbone=model_bk, RA_decoder=None)
    
    dict = torch.load(args.cart_resume_from)
    ######save complx_Angle_FFT paprameters#####
    # weights=dict["net_state_dict"]["complx_Angle_FFT.aoa_nn.weight.real"]
    # transposed_weights = weights.T  # 转置
    # np.savetxt("LR=0.001_FFT_real.txt", transposed_weights.cpu(), fmt="%.6f")
    #############################################
    ...
    # macs, params = get_model_complexity_info(model_cart, (8, 256, 64), as_strings=False,
...
################在RA_Cartesian_img上画出预测框与groundtruth框#############
    # pred_gt_on_cart_dataset = RararDatasetEvaluate(
    ...
    #     cart_shape=config_data["cart_shape"],
    #     dType="test",
    #     RDDir=config_data["input_type"]
    # )

    # draw_predAndgtOn_cart(config_data, model_cart,
    #                 radar_dataset=iter(pred_gt_on_cart_dataset),
    ...
    if if_evaluate_cart:
        ...
        print(len(test_loader_cart.dataset))
        average_time_per_sample = total_time / len(test_loader_cart.dataset)     #计算时间时，batchsize 设为 1 #
...
def get_parse():
    ...
    parser.add_argument("--resume_from", type=str,
                        default= None,#"/media/ljm/UserDisk/Dujialun/RADDet_Pytorch-main/logs/evaluate/RadarResNet/DJL-RAD-b_4-lr_0.0001/ckpt/best.pth",
                        help="The number of machines.")
    parser.add_argument("--cart_resume_from", type=str,
                        default="/media/sqpang/新加卷/rada_target_detection/code/Finalmodel_LinearFFT+Detection/logs/Para-init_A_FFT_flip_No_attention_FPN_RD_16_32_liner_RD_cartesian/ckpt/best.pth",
                        help="The number of machines.")
