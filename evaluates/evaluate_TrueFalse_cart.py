...
def get_GT_TF(predictions, gts, input_size, class_names, mode, tp_iou_threshold=0.3):
    """
    mode="3D":
...
def main(args):
    ...
    # model = RADDet(config_model, config_data, config_train, anchor_boxes)  #RADDet的源代码
    ...
    output_dir = os.path.join(logdir_cart, 'results_TrueFalse')
    ...
    test_loader_cart = DataLoader(test_dataset_cart,
                                  batch_size=config_evaluate["batch_size"]//args.num_gpus,
                                  shuffle=False,
                                  num_workers=4,
                                  pin_memory=True,
    ...
    for data, label, raw_boxes_cart, RAD_file_spec in test_loader_cart:
        data = data.to(device)
        ...
        for batch_id in range(raw_boxes_cart.shape[0]):
            pred_frame = pred[batch_id]
            RAD_file = RAD_file_spec[batch_id]
            raw_boxes_cart_1 = raw_boxes_cart[batch_id]
            ...
            dataID = os.path.splitext(RAD_file)[0]

            df = get_GT_TF(nms_pred_cart, raw_boxes_cart_1, config_model["input_shape"], config_data["all_classes"],
                       mode="2D")
            df.insert(0, 'Image_ID', dataID)
            df_2D_TF = pd.concat([df_2D_TF, df], ignore_index=True)

    filename = os.path.join(output_dir, RAD_dir + "-Cartesian Boxes GT TF_b=6_cft=0.6IOU=0.3.csv")
...
def get_parse():
    ...
    parser.add_argument("--cart_resume_from", type=str,
                        default="logs/Para-init_A_FFT_flip_No_attention_FPN_RD_16_32_liner_RD_cartesian/ckpt/best.pth",
                        help="The number of machines.")
